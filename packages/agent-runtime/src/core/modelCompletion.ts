...
/**
 * Thrown by a `call_llm` executor when the model returns an empty completion
 * — no text content, no reasoning, no tool calls, no images, and ~0 output
 * tokens. This is the "empty completion" failure mode: after a stalled tool loop
 * the model effectively gives up and emits a blank turn, which the harness used
 * to silently finalize to `done` while persisting an empty assistant message
 * (empty bubble, `status=done, error=null`).
 *
 * The `errorType` field tags it as the retryable `ModelEmptyCompletion` code so
 * that:
 *   1. an LLM-error classifier resolves it to `retry`, letting the executor's
 *      LLM retry loop re-attempt the turn (a retry typically yields real
 *      content).
 ...
 * Callers throw {@link ModelEmptyError} on a hit so the LLM retry loop
 * re-attempts instead of silently finalizing to `done` with a blank assistant
 * message.
 */
